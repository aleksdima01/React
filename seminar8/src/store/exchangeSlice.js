...
export const fetchExchange = createAsyncThunk('exchange/fetchExchange',
    async (_, thunkApi) => {              // {rejectWithValue,getState,dispatch}=thunkApi функции внутри thunkApi
        try {
            const { convertation } = thunkApi.getState();
            ...
            if (!convertation.currencyList.length) {
                thunkApi.dispatch(setCurrencyList(Object.keys(data.convertation_rates)));
                console.log(Object.keys(data.convertation_rates))
            }
            return data.convertation_rates  //попадет в payload 
        } catch (error) {
